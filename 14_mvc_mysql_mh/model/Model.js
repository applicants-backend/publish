...
// 단일연결
// 요청할때마다 새로운 연결을 생성
// 적은수의 동시연결이나 단순한 데이터 베이스 쿼리일때 사용

// createPool
// 연결 풀을 생성, 풀은 미리 정의된 수의 연결을 생성하고 관리
// 요청이 들어오면 연결 풀에서 사용가능한 연결을 제공, 작업 완료후 해당 연결 반환
// 연결이 필요하지 않을경우 자동으로 반환. 풀의 연결 수를 제한하고 관리르 최적화
// 다증연결 서비스에 적합
...
const conn = mysql.createPool({
    ...
    database: 'kdt9',
    port: 3306,
    connectionLimit : 30// 최대 연결수 (기본값 10)
});
...
// 회원 정보 수정
const db_edit_profile = (data, cb) =>{
    const query = `UPDATE user SET userid = ? , name = ?, pw = ?`
    conn.query(query, [data.userid,data.name,data.pw,data.id], (err) => {
        if (err) {
            console.log(err);
            return;
        }
        console.log('db_edit_profile', rows)
        cb();
    })
};
