...
const jwt = require('jsonwebtoken'); //  사용자 검증 툴
...
const edit_profile = async (req, res) => {
    ...
    //     res.json({ result: true });
    // });
    // headers의 요청은 req.headers 안에 있음.
    console.log(req.headers);
    // split() 는 () 안의 문자를 기준으로 문자열을 잘라내기한 후 배열을 반환
    const [bearer, token] = req.headers.authorization.split(" "); // 빈칸을 기준으로 split 한다.
    ...
    if ( bearer === "" ) {
        try {
            ...
            console.log(result);
            const returnValue = await User.findeOne({ where : {id : result.id}});
            if (returnValue) {
                const hash = await bcryptPassword(pw)
        ...
        res.json({result:false, message: '잘못된 인증 방식입니다.'})
    }
